...
import { DEFAULT_MANGA_T, REQUIRED_FIELDS } from "template/constant";
...
class TypeSelectionModal extends Modal {
	...
	onOpen() {
		...
		const typeDescriptions = {
			...
			number: { icon: "hash", name: "Number", description: "Numeric value. Use for scores, ratings, chapter count, etc." },
...
export function renderManga(this: SettingTab, containerEl: HTMLElement) {
	...
	new Setting(containerEl)
		...
		.addText((el) => {
			el
				...
				.setPlaceholder(DEFAULT_MANGA_T.fileNameT)
				.onChange(async (value) => {
					this.plugin.settings.mangaNoteT.fileNameT = value;
	...
	new Setting(containerEl)
		...
		.addText((el) => {
			el
				...
				.setPlaceholder(DEFAULT_MANGA_T.fileDir)
				.onChange(async (value) => {
					this.plugin.settings.mangaNoteT.fileDir = value;
					await this.plugin.saveSettings();
				});
			el.inputEl.setCssProps({flex: "1", minWidth: "200px", maxWidth: "400px"})
		});
	...
	const list = this.plugin.settings.mangaNoteT.frontMatterT;
	...
	for (let i = 0; i < list.length; i++) {
		...
		row.addEventListener("drop", (e) => {
			...
			if (fromIndex !== -1 && fromIndex !== toIndex) {
				// Reorder the array
				const movedItem = this.plugin.settings.mangaNoteT.frontMatterT[fromIndex];
				if (movedItem) {
					this.plugin.settings.mangaNoteT.frontMatterT.splice(fromIndex, 1);
					this.plugin.settings.mangaNoteT.frontMatterT.splice(toIndex, 0, movedItem);
		...
		new Setting(row)
			...
			.addButton((btn) => {
				btn.setIcon("x").onClick(async () => {
					this.plugin.settings.mangaNoteT.frontMatterT.splice(i, 1);
					await this.plugin.saveSettings();
					this.display();
				});
			});
	...
	resetButton.onclick = async () => {
		this.plugin.settings.mangaNoteT.frontMatterT = [...DEFAULT_MANGA_T.frontMatterT];
		await this.plugin.saveSettings();
		this.display();
	};
	...
	new Setting(containerEl)
		...
		.addTextArea((ta) => {
			ta
				.setPlaceholder(DEFAULT_MANGA_T.noteBodyT)
				.setValue(this.plugin.settings.mangaNoteT.noteBodyT)
				.onChange(async (value) => {
					this.plugin.settings.mangaNoteT.noteBodyT = value;
					await this.plugin.saveSettings();
				});
			ta.inputEl.rows = 6;
